...
const BankDetails = () => {
  ...
  useEffect(async () => {
    CustomerServices.getCurrentCustomer()
      .then((res) => {
        ...
        CustomerServices.getCustomerStatemment()
          .then((res) => {
            // console.log(res);
            ...
            setStatemment(
              [...res.payments, ...pp]
                ...
                .filter((item) => item.total !== 0 || item.amount !== 0)
  ...
  return (
    <Dashboard
      title="Store Statemment"
      description="You can see your store statemment"
    >
      ...
      <RecentStatemment data={statemment} loading={loading} error={false} />
    </Dashboard>
  );
};

export default dynamic(() => Promise.resolve(BankDetails), { ssr: false });
